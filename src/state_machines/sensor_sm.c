...
enum SensorState
{
    SENSOR_CHECK_LIGHT,      // 조도를 인식하는 상태
    SENSOR_CHECK_SOUND,      // 박수 소리를 인식하는 상태
    SENSOR_ADC_CHANGE_SOUND, // ADC 채널 변경 후 안정화 대기 상태
    SENSOR_ADC_CHANGE_LIGHT, // ADC 채널 변경 후 안정화 대기 상태
    SENSOR_STATE_COUNT
};

static enum SensorState sensor_mode = SENSOR_CHECK_LIGHT;
...
static void sensor_adc_change_sound(void);
static void sensor_adc_change_light(void);

static StateFuncNoParam state_table[SENSOR_STATE_COUNT] = {
    sensor_check_light,
    sensor_check_sound,
    sensor_adc_change_sound,
    sensor_adc_change_light};

void sensor_state_machine_initialize()
{
    initialize_done = 1;
    sensor_mode = SENSOR_CHECK_LIGHT;
...
void sensor_state_machine()
{
    ...
    state_table[sensor_mode]();
}

/*
    @brief
    SENSOR_ADC_CHANGE_SOUND 상태 함수
    잠시 대기 후 SENSOR_CHECK_SOUND 상태로 전환
...
static void sensor_adc_change_sound(void)
{
    if (watch_check(&adc_change_watch))
    {
        ...
        sensor_mode = SENSOR_CHECK_SOUND;
    }
}

/*
    @brief
    SENSOR_ADC_CHANGE_LIGHT 상태 함수
    잠시 대기 후 SENSOR_CHECK_LIGHT 상태로 전환
    ADC 채널 변경 후 안정화 시간 확보
*/
static void sensor_adc_change_light(void)
{
...
static void sensor_check_sound(void)
{
    ...
    if (clap_state_machine_finished())
    {
        ...
        adc_init(ADC_CHANNEL_CDS);
        sensor_mode = SENSOR_ADC_CHANGE_LIGHT;
    }
...
static void sensor_check_light(void)
{
    ...
    watch_start(&adc_change_watch);
    sensor_mode = SENSOR_ADC_CHANGE_SOUND;
}
