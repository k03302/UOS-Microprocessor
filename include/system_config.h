...
enum SystemAttribute
{
    ...
    근접 박수소리: 800~900
    */
    SA_SOUND_THRESHOLD = 0,           // 현재 설정된 박수소리 역치 (adjustable)
    SA_SOUND_THRESHOLD_DISPLAY,       // 현재 표시되고 있는 박수소리 역치 (adjustable)
    SA_SOUND_THRESHOLD_MIN,           // 설정 가능한 박수소리 역치의 최솟값
    SA_SOUND_THRESHOLD_MAX,           // 설정 가능한 박수소리 역치의 최댓값
    SA_SOUND_THRESHOLD_ADJUST_AMOUNT, // knob을 돌릴 때 박수소리 역치를 조절하는 단위

    /*
    광감지센서 상수
    value: 0 ~ 1023
    */
    SA_CDS_NIGHT_THRESHOLD, // 밤으로 전환하는 조도 역치
    ...
    SA_FND_UPDATE_TIMEOUT,    // 박수소리 역치 디스플레이를 끝내는 시간 (msec)

    SA_ADC_CHANGE_TIMEOUT, // ADC 입력원을 변경할 때의 대기시간 (msec)

    SA_SYSTEM_ATTRIBUTE_END
};
